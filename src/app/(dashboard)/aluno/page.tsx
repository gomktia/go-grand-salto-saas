...
import { getPerfilAluno, getAgendaAluno, getMetricasAluno } from '@/app/actions/portal-aluno'
...
export default function AlunoDashboard() {
    ...
    async function loadData() {
        try {
            ...
            const [perfilRes, aulasRes, metricasRes] = await Promise.all([
                getPerfilAluno(),
                getAgendaAluno(),
                getMetricasAluno()
            ])
            ...
            setMetricas(metricasRes.data as Metricas)
        } catch (err) {
                ...
                <div className="flex items-center gap-6">
                    ...
                    <div className="space-y-1">
                        ...
                        <p className="text-muted-foreground font-black uppercase text-xs tracking-widest flex items-center gap-2">
                            <Sparkles className="w-4 h-4" /> {tenant?.nome} {perfil?.turmas?.[0] && <>• <span className="text-foreground/60">{perfil.turmas[0].nivel}</span></>}
                        </p>
                    </div>
                </div>
                    ...
                    <div className="flex items-center gap-4 p-5 rounded-3xl bg-card border border-border shadow-sm">
                        ...
                        <div>
                            ...
                            <p className="text-lg font-black tracking-tighter uppercase">
                                {perfil?.status_matricula === 'ativo' ? 'Ativo' : perfil?.status_matricula || 'N/A'}
                            </p>
                        </div>
                    </div>
                    <div className="flex items-center gap-4 p-5 rounded-3xl bg-card border border-border shadow-sm">
                        ...
                        <div>
                            ...
                            <p className="text-lg font-black tracking-tighter uppercase">{metricas?.streak || 0} Dias</p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <Card className="lg:col-span-2 bg-card border-border shadow-sm rounded-[3rem] overflow-hidden">
                    <CardHeader className="p-10 border-b border-border/50 bg-muted/20">
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <CardDescription className="text-xs font-black uppercase tracking-widest text-muted-foreground mt-2 ml-16">Organizacao e Disciplina para o Palco</CardDescription>
                            </div>
                            <Button
                                onClick={() => toast.info('A agenda completa sera liberada na proxima atualizacao.')}
                                variant="outline"
                                className="h-10 px-6 rounded-xl border-border font-black text-[10px] uppercase tracking-widest hover:bg-muted transition-all"
                            >
                                Ver Mes Todo
                            </Button>
                        </div>
                    ...
                    <CardContent className="p-10 space-y-4">
                        {aulas.length === 0 ? (
                            ...
                            aulas.slice(0, 3).map((aula, i) => {
                                const isHoje = aula.dia_semana === new Date().getDay()
                                return (
                                    <div key={aula.id} className="flex flex-col md:flex-row md:items-center justify-between p-8 rounded-[2.5rem] bg-muted/20 border-2 border-transparent hover:border-[var(--primary)]/20 transition-all group cursor-pointer relative overflow-hidden">
                                        <div className="flex items-center gap-8">
                                            <div className="text-center min-w-[70px] space-y-1">
                                                <p className="text-xl font-black tracking-tighter" style={{ color: primaryColor }}>{aula.hora_inicio?.slice(0, 5)}</p>
                                                <p className="text-[10px] text-muted-foreground font-black uppercase tracking-widest">{isHoje ? 'Hoje' : diasSemana[aula.dia_semana]}</p>
                                            </div>
                                            <div className="h-12 w-px bg-border hidden md:block" />
                                            <div className="space-y-1">
                                                <div className="flex items-center gap-2">
                                                    <p className="font-black text-lg uppercase tracking-tight">{aula.turma?.nome}</p>
                                                    <Badge variant="outline" className="text-[8px] font-black uppercase tracking-widest border-none bg-emerald-500/5 text-emerald-500">
                                                        {aula.turma?.nivel}
                                                    </Badge>
                                                </div>
                ...
                <div className="space-y-10">
                    <Card className="bg-card border-border shadow-sm rounded-[3rem] overflow-hidden group">
                        <CardHeader className="p-10 pb-6">
                            <CardTitle className="text-[11px] font-black uppercase tracking-[0.3em] flex items-center gap-3 text-muted-foreground">
                                ...
                                Roadmap de Aperfeicoamento
                            </CardTitle>
                        </CardHeader>
                        <CardContent className="p-10 pt-0 space-y-8">
                            {[
                                { label: 'Tecnica e Precisao', val: metricas?.tecnica || 50, icon: Sparkles, color: primaryColor },
                                { label: 'Musicalidade', val: metricas?.musicalidade || 50, icon: Music, color: '#f59e0b' },
                                { label: 'Presenca de Palco', val: metricas?.presenca_palco || 50, icon: Zap, color: '#8b5cf6' },
                            ].map((skill, i) => (
                    ...
                    <Card className="bg-neutral-900 border-none shadow-2xl rounded-[3rem] overflow-hidden group relative min-h-[350px] flex items-center">
                        ...
                        <CardContent className="p-10 text-center relative z-10 w-full">
                            ...
                            <p className="text-[10px] text-neutral-400 font-black uppercase tracking-widest mb-8 leading-relaxed max-w-[180px] mx-auto opacity-70">Acesse a galeria de fotos dos seus eventos!</p>
                            <Button
                                onClick={() => toast.info('A galeria de fotos sera liberada na proxima atualizacao.')}
